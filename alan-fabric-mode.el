...
(defconst alan-fabric-mode-font-lock-keywords
  `(
	'(
	  ...
	  (,(regexp-opt '("!" "#" "$" "%" "%^" "%}" "&" "(" ")" "*" "*&" "*&#" "+" "+^" "-" "->" ".>"
					  "~>" "." ".^" ":" "<" "<=" "==" ">" ">=" "?" "?^" "@" "add" "allow"
					  "anonymous" "as" "branch" "ceil" "command" "component" "constrain" "contains"
					  "convert" "count" "creation" "densematrix" "derived" "dictionary" "division"
					  "dynamic" "empty" "execute" "external" "floor" "forward" "from" "group" "id"
					  "implementation" "in" "increment" "integer" "interface" "lifetime" "link"
					  "log" "map" "matrix" "max" "min" "mutation" "natural" "no" "now" "number"
					  "numerical" "on" "ontimeout" "or" "ordered" "password" "path" "product"
					  "property" "read" "reference" "remainder" "remove" "rename" "requires"
					  "resolvable" "roles" "root" "set" "setto" "sparsematrix" "stategroup"
					  "stopwatch" "subtract" "sum" "sumlist" "text" ".self" "time" "timer" "to"
					  "types" "unconstrained" "update" "users" "with" "workfor" "yes" "new" "~" "}"
					  "/" "{" "@?^" "@%^" "^" "`" "=" "ignore" "file" "create" "delete" "created"
					  "entry" "acyclic" "space" "TEMP" "TEMP2" "relative" "preceding" "base" "diff"
					  "std" "deprecated" "zero" "one" "$^" "bidirectional" "<~" "invalidate"
					  "referencer" "separator" "FIXME"))
